...
_SYNTHESIS_PATTERNS = [
    ...
    re.compile(r"\b(qu[ae]ll?es?\s+sont\s+(les?|tous\s+les?|toutes\s+les?))\s+(produits?|articles?|prestations?|services?|offres?|prix|tarifs?)\b"),
    ...
    re.compile(r"\b(dis[- ]?moi|montre[- ]?moi|explique[- ]?moi)\s+(tout|la\s+liste|le\s+catalogue|les?\s+(produit|prestation|service))\b"),
    re.compile(r"\btout\s+ce\s+(que\s+(vous|tu)\s+(avez?|vendez?|proposez?)|qui\s+est\s+disponible)\b"),
    re.compile(r"\b(quels?\s+(produits?|articles?|prestations?|services?|options?|accessoires?)|qu'est[- ]ce\s+que\s+vous?\s+vendez?)\b"),
